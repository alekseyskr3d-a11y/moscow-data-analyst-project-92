...
HAVING FLOOR(AVG(products.price * sales.quantity)) < (
    ...
    JOIN products AS products_inner ON sales_inner.product_id = products_inner.product_id
)
...
first_purchase_details AS (
    ...
    JOIN sales ON first_purchases.customer_id = sales.customer_id
...
JOIN customers ON first_purchase_details.customer_id = customers.customer_id
JOIN employees ON first_purchase_details.sales_person_id = employees.employee_id
